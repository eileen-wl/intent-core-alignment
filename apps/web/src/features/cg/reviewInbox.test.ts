...
describe("adaptCgCurrentFocusToWorkItems", () => {
  ...
  const actionableTypes: {
    ...
    {
      focusType: "execution_anchor_gate_pending",
      category: "Execution Anchor confirmation",
      route: "/cg/tasks/t1/execution",
    },
    {
      focusType: "execution_anchor_draft_needs_review",
      category: "Draft review",
      route: "/cg/tasks/t1/execution",
    },
    {
      focusType: "dependency_needs_attention",
      category: "Dependency review",
      route: "/cg/tasks/t1/dependencies",
    },
    {
      focusType: "version_review_available",
  ...
  it.each(actionableTypes)(
    ...
    ({ focusType, category, route }) => {
      const workItems = adaptCgCurrentFocusToWorkItems([
        item({
          task_id: "t1",
          current_focus: focus(focusType, { target_route: route }),
        }),
      ]);
      expect(workItems).toHaveLength(1);
  ...
  it("uses a stable id namespaced by source type and focus type, never taskId alone", () => {
    ...
    expect(workItems[0].id).toBe(
      "current_focus:t1:execution_anchor_gate_pending",
    );
  ...
  it("carries real Project/Shot/Task context, never fabricated", () => {
    ...
    expect(workItems[0].task).toEqual({
      id: "t1",
      name: "Lighting Pass",
      department: "lighting",
    });
