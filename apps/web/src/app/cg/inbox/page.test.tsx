...
vi.mock("@/features/cg/api", () => ({
  ...
  fetchCgAnchorContextSummaries: vi.fn(async () => ({
    items: [],
    total_count: 0,
    limit: 200,
  })),
...
describe("/cg/inbox page", () => {
  ...
  it("passes the real loaded inbox through to CgReviewInboxPage", async () => {
    const inbox = { items: [], generated_at: "2026-01-01T00:00:00Z" };
    fetchCgInboxMock.mockResolvedValue(inbox);

    const result = await Page();

    expect(result.type).toBe(CgReviewInboxPage);
