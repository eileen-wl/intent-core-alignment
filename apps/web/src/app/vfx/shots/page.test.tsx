...
describe("/vfx/shots page", () => {
  ...
  it("renders ShotsListPage with the fetched inbox on success", async () => {
    const inbox = { items: [], generated_at: "2026-01-01T00:00:00Z" };
    fetchVfxInboxMock.mockResolvedValue(inbox);

    const result = await Page();

    expect(result.type).toBe(ShotsListPage);
    expect(result.props.inbox).toBe(inbox);
  });

  it("renders ShotsListPage with a null inbox when the fetch fails, rather than throwing", async () => {
