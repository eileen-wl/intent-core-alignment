  ...
  return (
    ...
    >
      {unavailable || data === null ? (
        <>
          <Breadcrumbs items={[{ label: "Tasks", href: "/artist/tasks" }, { label: "Task" }]} />
          <ErrorState
        ...
        <>
          ...
          />
          <Panel>
            <AuthorityLabel variant="read-only" detail="Confirmed by the VFX Supervisor" />
            {data.coreAnchorRevision ? (
              <MetadataRow
                items={[
                  { label: "Shot objective", value: data.coreAnchorRevision.shot_objective ?? "Not recorded" },
                  { label: "Emotional tone", value: data.coreAnchorRevision.emotional_tone ?? "Not recorded" },
                  { label: "Narrative priority", value: data.coreAnchorRevision.narrative_priority ?? "Not recorded" },
                  { label: "Core summary", value: data.coreAnchorRevision.core_summary ?? "Not recorded" },
                ]}
              />
            ) : (
          ...
          />
          <Panel>
            <AuthorityLabel variant="read-only" detail="Confirmed by the CG Supervisor" />
            {data.executionAnchorRevision ? (
              <MetadataRow
                items={[
                  {
                    label: "Technical boundaries",
                    value: data.executionAnchorRevision.technical_boundaries ?? "Not recorded",
                  },
                  {
                    label: "Allowed refinements",
                    value: data.executionAnchorRevision.allowed_refinements ?? "Not recorded",
                  },
                  {
                    label: "Escalation conditions",
                    value: data.executionAnchorRevision.escalation_conditions ?? "Not recorded",
                  },
                ]}
              />
            ) : (
            ...
            actions={
              data.item.latest_version_id && (
                <GenerateArtistGuidanceButton
                  ...
                  label={data.latestGuidance ? "Regenerate guidance" : "Generate guidance"}
                />
              )
            }
          />
          <Panel tone="elevated">
            <StatusBadge
              status={data.item.guidance_state === "outdated" ? "attention" : "neutral"}
            ...
            {data.latestGuidance ? (
              <div>
                ...
                {data.item.guidance_state === "outdated" && (
                  <p>
                    This guidance references an earlier confirmed Execution Anchor revision. Regenerate
                    it to reflect the current one.
                  </p>
                )}
                <h3>What must remain fixed</h3>
                {data.latestGuidance.guidance_output.non_negotiables.length === 0 ? (
                  <p>No non-negotiable constraints were identified.</p>
                ) : (
                  <ul>
                    {data.latestGuidance.guidance_output.non_negotiables.map((it, index) => (
                      <li key={index}>{it.summary}</li>
                    ))}
                  </ul>
                )}
                <h3>What variation remains allowed</h3>
                {data.latestGuidance.guidance_output.allowed_variations.length === 0 ? (
                  <p>No allowed variation was identified.</p>
                ) : (
                  <ul>
                    {data.latestGuidance.guidance_output.allowed_variations.map((it, index) => (
                      <li key={index}>{it.summary}</li>
                    ))}
                  </ul>
                )}
                <h3>Priorities for the next iteration</h3>
                {data.latestGuidance.guidance_output.iteration_priorities.length === 0 ? (
                  <p>No iteration priorities were identified.</p>
                ) : (
                  <ul>
                    {data.latestGuidance.guidance_output.iteration_priorities.map((it, index) => (
                      <li key={index}>{it.summary}</li>
                    ))}
                  </ul>
                )}
                <h3>Risks and when to escalate</h3>
                {data.latestGuidance.guidance_output.cross_department_dependencies.length === 0 &&
                data.latestGuidance.guidance_output.questions_for_human_supervisor.length === 0 ? (
                  <p>No risks or escalation conditions were identified.</p>
                ) : (
                  <ul>
                    {data.latestGuidance.guidance_output.cross_department_dependencies.map((it, index) => (
                      <li key={`dep-${index}`}>{it.summary}</li>
                    ))}
                    {data.latestGuidance.guidance_output.questions_for_human_supervisor.map((q, index) => (
                      <li key={`q-${index}`}>{q}</li>
                    ))}
                  </ul>
                )}
