...
describe("ArtistWorkspacePage", () => {
  it("renders the correct App Shell with fixed Artist identity", () => {
    render(<ArtistWorkspacePage inbox={buildInbox([buildItem()])} onExitRole={vi.fn()} />);
  ...
  it("Priority actions leads with the required action, never the Task name, and contains at most 3 items", () => {
    ...
    render(<ArtistWorkspacePage inbox={buildInbox(items)} onExitRole={vi.fn()} />);
  ...
  it("Priority actions opens guidance work in the real Task Overview route", () => {
    render(
      <ArtistWorkspacePage inbox={buildInbox([buildItem({ task_id: "t1" })])} onExitRole={vi.fn()} />,
    );
    const priorityActions = within(screen.getByRole("region", { name: "Priority actions" }));
    const link = priorityActions.getByText("Artist guidance is outdated").closest("a");
  ...
  it("does not fabricate a task_name or supervisor-only Task label as Priority action text", () => {
    render(<ArtistWorkspacePage inbox={buildInbox([buildItem({ task_id: "t1" })])} onExitRole={vi.fn()} />);
    expect(screen.queryByText(/supervisor clarification pending/i)).not.toBeInTheDocument();
  });

  it("wires Exit role view to the provided callback", async () => {
    const onExitRole = vi.fn();
    render(<ArtistWorkspacePage inbox={buildInbox([buildItem()])} onExitRole={onExitRole} />);
