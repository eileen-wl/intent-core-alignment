...
| | Core Anchor establishment | **Partial** — drafting + human confirm/edit only | `core_anchor_drafting` capability exists (the seven scalar fields only); confirmation, and now full semantic-content authorship (Must preserve/Allowed variation/High-risk drift points/References/Open questions, Step 1A), are human acts via the UI, not Agent capabilities. Establishment remains partial specifically because Core Agent cannot yet decompose intent into these semantic collections itself — that is Step 1B Intent Decomposition, still not started; today the Agent only drafts the seven scalar fields, and a human populates everything else. |
...
1. Core Anchor semantic objects
   ...
   among 0-9):
     ...
     1B          Core Agent Intent Decomposition                [NEXT — not started]
     1C          Core Agent Context Reconstruction               [not started]
     1D          Minimal persistent HumanGate                    [not started]

   Step 1A as a whole (1A-Backend + 1A-UI) is complete on the feature
   branch, pending merge to main. Step 1B must not begin until, in order:
   (1) the Step 1A-UI/docs commit is made; (2) the feature branch is
   pushed; (3) Step 1A is merged into main; (4) local main is
...
- **Current status (sub-sliced; see section H for the locked internal order):**
  ...
  - **Step 1A as a whole (1A-Backend + 1A-UI) is complete on `feat/step1a-core-anchor-semantics`, pending merge to `main`.** Step 1 overall is *not* complete — Step 1B/1C/1D remain.
  - **Step 1B (Core Agent Intent Decomposition), Step 1C (Core Agent Context Reconstruction), Step 1D (minimal persistent HumanGate) — not started.** Step 1B is the immediate next locked step, but none of 1B/1C/1D may begin until, in order: the Step 1A-UI/docs commit is made; the feature branch is pushed; Step 1A is merged into `main`; local `main` is synchronised and clean.
