...
|---|---|---|
| **Core Agent** | Implemented (3 of ~6 documented capabilities) | `agents/core_agent_service.py` (`core_anchor_drafting`), `agents/alignment_assessment_service.py` (`alignment_assessment`), `agents/intent_decomposition_service.py` (`intent_decomposition`, Step 1B — implemented and validated on `feat/step1b-intent-decomposition`, pending merge to `main`); all three use `ContextSnapshot`/`AgentRun`; deterministic provider for all three, real DeepSeek provider proven for Alignment Assessment and Intent Decomposition |
...
|---|---|---|---|
| **Core Agent** | Intent decomposition | **Complete on feature branch, pending merge to main** — implemented and validated on `feat/step1b-intent-decomposition` | `agents/intent_decomposition_service.py`, migration `0015_intent_decomposition.py`; deterministic provider for automated tests, real DeepSeek provider proven by manual acceptance (`docs/VALIDATION_EVIDENCE.md`) |
| | Core Anchor establishment | **Partial** overall — Step 1A semantic objects complete; Step 1B decomposition-led upstream proposal complete (both pending merge); wider Step 1 remains Partial because Context Reconstruction (Step 1C) and the minimal persistent HumanGate (Step 1D) are not implemented | `core_anchor_drafting` (direct path, still backend-compatible but demoted in the UI) plus the new Step 1B decomposition-led path (`intent_decomposition` → Human VFX Supervisor review → explicit "Use in Core Anchor draft" → editable `CoreAnchorRevision` draft with `source_intent_decomposition_id` lineage) both exist; confirmation and full semantic-content authorship remain human acts via the UI (Step 1A). Establishment is not yet complete because Core Agent Context Reconstruction and the minimal persistent HumanGate (Step 1C/1D) are still not started. |
| | Core Anchor semantic supporting objects | **Complete** (backend + minimal UI, Step 1A) | `Constraint`/`VariationZone`/`DriftRisk`/`AnchorReference`/`OpenQuestion` (migration `0014`, committed `173f32e`) plus the Human VFX Supervisor view/add/edit/remove/move/save/cancel UI inside the existing Core Anchor draft workflow (`CoreAnchorGate`, `feat/step1a-core-anchor-semantics`, pending merge to `main`); human-authored only — Core Agent does not yet populate these (see Intent decomposition row above) |
...
1. Core Anchor semantic objects
   ...
   among 0-9):
     1A-Backend  Semantic objects backend                    [DONE — committed 173f32e]
     1A-UI       Minimal Core Anchor semantic editors          [DONE — implemented + validated on feat/step1a-core-anchor-semantics, pending merge to main]
     1B          Core Agent Intent Decomposition                [DONE — implemented + validated on feat/step1b-intent-decomposition, pending merge to main]
     1C          Core Agent Context Reconstruction               [NEXT — not started]
     1D          Minimal persistent HumanGate                    [not started]

   Step 1A (1A-Backend + 1A-UI) and Step 1B are both complete on their
   respective feature branches, pending merge to main. Step 1C must not
   begin until, in order: (1) the Step 1B docs commit is made; (2) the
   feature branch is pushed; (3) Step 1A and Step 1B are merged into
   main; (4) local main is synchronised and clean.
...
- **Current status (sub-sliced; see section H for the locked internal order):**
  ...
  - **Step 1A-UI — implemented and validated on `feat/step1a-core-anchor-semantics`, pending merge to main.** The Human VFX Supervisor can view/add/edit/remove/move up/move down/save/cancel all five semantic collections on a draft revision, inside the existing `CoreAnchorGate` workflow; Human CG Supervisor and Human Artist see a read-only "Draft details" view instead of a disabled form; confirmed revisions render all five collections read-only. Automated evidence: 70 frontend tests passing (54 Shot Anchor + 16 Version page), web TypeScript typecheck/ESLint/Prettier/production build all passing, no backend/migration/contract/generated/Version-page/dependency file touched. Manual evidence: a real-browser acceptance pass — see section E item 7 and `docs/VALIDATION_EVIDENCE.md`.
  - **Step 1A as a whole (1A-Backend + 1A-UI) is complete on `feat/step1a-core-anchor-semantics`, pending merge to `main`.**
  - **Step 1B (Core Agent Intent Decomposition) is complete on `feat/step1b-intent-decomposition`, pending merge to `main`.** Core Agent generates an immutable `IntentDecomposition` from an `IntentBrief`; the Human VFX Supervisor reviews the AI proposal and may explicitly use it to create a new, fully editable `CoreAnchorRevision` draft (migration `0015_intent_decomposition.py`). See sections D and E above and `docs/VALIDATION_EVIDENCE.md` for the full automated, real-DeepSeek, and browser evidence.
  - **Step 1 overall is *not* complete** — Step 1C (Core Agent Context Reconstruction) and Step 1D (minimal persistent HumanGate) remain not started.
  - **Step 1C (Core Agent Context Reconstruction) is the immediate next locked step.** It, and Step 1D after it, may not begin until, in order: the Step 1B docs commit is made; the feature branch is pushed; Step 1A and Step 1B are merged into `main`; local `main` is synchronised and clean. Step 1C is not planned or implemented by this document.
