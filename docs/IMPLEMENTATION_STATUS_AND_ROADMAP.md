...
| Step 1A-Backend Core Anchor semantic objects | Pending merge to main — implemented and validated on `feat/step1a-core-anchor-semantics` | Human authority / shared domain (Intent module) | Five ordered, revision-owned semantic-child tables (`Constraint`, `VariationZone`, `DriftRisk`, `AnchorReference`, `OpenQuestion`; migration `0014_core_anchor_semantic_objects.py`) exist, each belonging to exactly one `CoreAnchorRevision`, writable only while that revision is `draft`, immutable once confirmed/rejected/superseded, returned as empty arrays when absent, with real-HTTP-validated PATCH replace/omit/clear semantics and meaningful (not count-only) before/after `AuditEvent` content | Does not prove Step 1A-UI exists (no editor surface yet — the next sub-slice); does not prove Core Agent Intent Decomposition, Core Agent Context Reconstruction, or a persistent `HumanGate` object exist (all still not started); does not prove any Role Agent exists |
...
|---|---|---|
| **WP-A — Intent Core & Workflow** | Partial | Completed: Core Anchor lifecycle (A1), Execution Anchor lifecycle (A2), stale propagation, Decision/WorkflowTransition/AuditEvent foundations, Human Gate UI for Core Anchor (A3), Version/ReviewNote (Step 4a), Alignment Assessment + its Human Gate + supersession (Step 4b/4c), Version/Assessment UI (Step 4d), Core Anchor semantic objects **backend** — `Constraint`/`VariationZone`/`DriftRisk`/`AnchorReference`/`OpenQuestion` (Step 1A-Backend, migration `0014`, implemented and validated on `feat/step1a-core-anchor-semantics`, pending merge to main). Not started: Core Anchor semantic objects **UI** (Step 1A-UI — the immediate next sub-slice, see section H/I); Execution Anchor's own frontend Human Gate UI (backend exists, UI does not). |
...
1. Core Anchor semantic objects
   ...
   among 0-9):
     1A-Backend  Semantic objects backend                    [DONE — implemented + validated on feat/step1a-core-anchor-semantics, pending merge to main]
     1A-UI       Minimal Core Anchor semantic editors          [NEXT — not started]
     1B          Core Agent Intent Decomposition                [not started]
     1C          Core Agent Context Reconstruction               [not started]
     1D          Minimal persistent HumanGate                    [not started]

   Step 1B must not begin before Step 1A-UI is implemented and validated.
