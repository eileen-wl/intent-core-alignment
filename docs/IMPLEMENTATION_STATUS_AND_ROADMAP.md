...
- Core Anchor establishment is now **complete on the Step 1D feature branch, pending merge to `main`** — drafting (direct and decomposition-led), full semantic-content authorship, and the minimal persistent HumanGate all exist; this is a feature-branch completeness claim, not a claim that any of it is merged beyond what section D records.
...
1. Core Anchor semantic objects
   ...
   among 0-9):
     ...
     1D          Minimal persistent HumanGate                    [DONE — implemented + validated on feat/step1d-persistent-human-gate, pending merge to main]

   Step 1A, Step 1B, and Step 1C are all merged to main. Step 1D is
   complete on its feature branch, pending merge to main. With all four
   sub-slices done, Step 1 as a whole is complete on the feature branch,
   pending merge to main. Step 2 must not begin until, in order: (1) the
   Step 1D docs commit is made; (2) the feature branch is pushed; (3)
   Step 1D is merged into main; (4) local main is synchronised and
   clean.
...
- **Current status (sub-sliced; see section H for the locked internal order):**
  ...
  - **Step 1D (Minimal Persistent HumanGate) is complete on `feat/step1d-persistent-human-gate`, pending merge to `main`.** A first-class `human_gates` table (migration `0017_persistent_human_gate.py`) records that an authoritative human review is required for a `CoreAnchorRevision` draft and how it was resolved; every draft-creation path opens exactly one pending gate atomically with the draft; the existing Core Anchor confirm/reject actions remain the sole authoritative write path and now atomically resolve the gate, linking it to the existing `Decision` they already create; `Decision` remains the sole authoritative human record. See sections D and E above and `docs/VALIDATION_EVIDENCE.md` for the full automated, runtime-preparation, and browser evidence.
  - **Step 1 overall (1A + 1B + 1C + 1D) is complete on the feature branch, pending merge to `main`.** This is a feature-branch completeness claim, not a claim that Step 1 as a whole has been merged — see section D for the exact branch/merge state of each sub-slice.
  - **Step 2 (Lightweight shared Agent Runtime) is the immediate next locked step.** It may not begin until, in order: the Step 1D docs commit is made; the feature branch is pushed; Step 1D is merged into `main`; local `main` is synchronised and clean. Step 2 is not planned or implemented by this document.
