...
**New focused test files (counts as of the §19 owner-validation correction):**
...
- `apps/web/src/lib/decisionProvenance.test.ts` (6 tests) — real actor/rationale/timestamp with a human-readable role label, honest no-rationale fallback, supersession without a raw id, and `decisionOutcomeStatement`'s confirm/reject/Core-Anchor outcomes.
...
- `apps/web/src/lib/humanRoleLabel.test.ts` (4 tests) — every real `HumanRole` formats correctly, a stray mixed-case variant normalises the same way, an unrecognised value falls back to itself, and a missing value returns an honest "Unknown".
- `apps/web/src/lib/authorProvenance.test.ts` (6 tests — new this correction) — a real internal `HumanRole` formats as its readable label; no raw enum renders; a non-`HumanRole` `actor_kind` fallback stays unconverted; a real ftrack external author name stays source provenance, never converted into an ICAS role; an unrecognised role value passes through unchanged; an arbitrary ftrack name (even one that shares a substring with a role word) is never falsely converted.
- `apps/web/src/lib/feedbackEventSummary.test.ts` (5 tests — new this correction) — composes a readable confirm/reject Decision description from `event_type`/`actor_human_role`; leaves every other event type's summary (including ReviewNote content) completely unchanged; falls back to the original summary, unguessed, when the expected raw-role prefix is absent or `actor_human_role` is null.

**Tests added/updated in existing page-component test files, across the original 9B-2 pass and both correction rounds** (all passing alongside every pre-existing test in the same file, none weakened or deleted):
...
- `ConfirmedAnchorSummary.test.tsx` (18 total): the honest "No evidence references were recorded for this Decision." state; the main card's "Confirmed by" role renders as a human-readable label, never the raw enum.
- `AlignmentWorkspacePage.test.tsx` (15 total — +1 this correction): Version/Anchor reference under Production Evidence; the CrossRoleAssessment summary and its `AI interpretation` badge render under Agent Interpretation and never under Production Evidence; the honest no-Decision statement under Human Decision and Provenance; `human-review-required` renders inside Agent Interpretation's Recommended next action, never inside Human Decision and Provenance; a Finding's "Affects" list renders human-readable role labels ("VFX Supervisor, CG Supervisor, Artist"), never the raw enums.
- `ExecutionPage.test.tsx` (13 total): a real Decision's actor/rationale render under Human Decision and Provenance with a human-readable role label and a concise outcome statement ("Confirmed Execution Anchor revision N"), never duplicated in Production Evidence; the legacy fallback to the revision's own confirmed-by/at fields (also human-readable) when no Decision is found, with no fabricated outcome statement; a human-readable supersession note; the state-dependent action heading (Start / Revise / Draft Execution Anchor) for all three real states.
- `VersionReviewPage.test.tsx` (13 total): Production Evidence/Agent Interpretation grouping and the honest Human Decision statement (never manufactured); Add Review Note, Generate CG Supervisor review, and Escalate to VFX all render inside a separate "Review actions" section, outside every evidence layer, and remain reachable.
- `CurrentVersionPage.test.tsx` (20 total — +1 this correction): Agent Interpretation groups guidance/reviews/assessments; Human Decision shows only "Confirmed authority references" wording with no actor/rationale/decided-at detail and no confirm/reject/edit control; an unconfirmed Anchor is never described as confirmed; the manual Human-role author (e.g. `artist`) now renders as its human-readable label ("Artist"), never the raw enum.
- `FeedbackHistoryPage.test.tsx` (10 total — +1 this correction): chronology (newest-first) preserved across mixed-layer events; classification badge matches `event_type`, including the human-authored-but-Production-Evidence case; a human actor's role renders as a human-readable label, never the raw enum; a confirm/reject Execution Anchor Decision event's description composes from structured fields ("CG Supervisor confirmed Execution Anchor Revision 2 -- ..."), never the raw `Human cg_supervisor` text the server generates.
- `VersionsWorkspacePage.test.tsx` (13 total — +2 this correction, VFX Versions page, a beneficiary of the shared `authorProvenance.ts` fix though not one of the six priority pages): the manual Human-role Review Note/Version author now renders as a human-readable label, never the raw enum.

**Full regression, all green (as of the §19 owner-validation correction):**

- Frontend: Vitest 964/964 (123 files), ESLint (0 errors, 1 pre-existing/unrelated warning), `tsc --noEmit` (apps/web and contracts package, both clean), Prettier (clean, repo-root), production `next build` (30 routes, succeeded).
- Backend: no Python source was changed in this step or either correction round — no backend/contract test re-run was required or performed, per the task's own instruction to run API/contract tests only when an approved additive contract change is made (none was, in any of the three passes).
- Every test that existed before the §19 correction still passes unmodified, plus the new tests listed above — confirmed by running the complete suite both before and after this correction's changes.
...
## 19. Owner-validation correction (third pass) — residual raw role-enum leakage

**The second owner visual validation attempt confirmed the §18 classification/outcome/placement/authority-boundary corrections, but found that raw `HumanRole` enum values remained visible in four further locations** — directly contradicting §18.6's own "zero matches" claim, which was true for the four spots it explicitly checked but did not cover the whole surface the task named. This section records each remaining location, its correction, and a broader re-verification than §18.6 performed.

### 19.1 Root cause: two distinct code paths, not one

§18 fixed every location that already called (or was changed to call) `humanRoleLabel`/`decisionProvenanceItems` directly. It missed two further code paths that render a role or an event description without going through either:

1. **`apps/web/src/lib/authorProvenance.ts`'s `getAuthorDisplayText`** — the shared Version/ReviewNote author-display helper, used by CG Version Review, Artist Current Version, and (incidentally) the VFX Versions page. For an internal/manual record it returned `created_by_human_role ?? created_by_actor_kind` verbatim — the raw enum, uncorrected.
2. **The backend-generated `summary` string on `execution_anchor_confirmed`/`execution_anchor_draft_discarded` Feedback History events** (`apps/api/.../artist_feedback_history/service.py`'s `f"Human {role_label} {verb} Execution Anchor Revision {revision.revision_number} -- ..."`, where `role_label = decision.actor_human_role`) — a persisted-text-embeds-the-enum case, not a separate label field the frontend could simply reformat.

### 19.2 VFX Alignment: Finding "Affects" list

**Symptom:** `finding.affected_roles.join(", ")` rendered `"Affects: vfx_supervisor, cg_supervisor, artist"` verbatim — `affected_roles` is a `CrossRoleRole` list, the exact same three-value enum as `HumanRole`, never mapped through any label function.

**Correction, `apps/web/src/app/vfx/shots/[shotId]/alignment/AlignmentWorkspacePage.tsx`:** `finding.affected_roles.map(humanRoleLabel).join(", ")` — now renders `"Affects: VFX Supervisor, CG Supervisor, Artist"`.

### 19.3 CG Version Review / Artist Current Version (+ VFX Versions): ReviewNote/Version author provenance

**Symptom:** an internal/manual ReviewNote or Version's author rendered as e.g. `"vfx_supervisor"` — the shared `getAuthorDisplayText` helper's non-ftrack branch.

**Correction, `apps/web/src/lib/authorProvenance.ts`:** the non-ftrack branch now returns `humanRoleLabel(entity.created_by_human_role)` when a real human role is recorded, falling back to the unconverted `created_by_actor_kind` (e.g. `"agent"`, `"system"` — never a `HumanRole`, so never passed through the formatter) when it is not. The ftrack branch (`external_author_name`) is completely untouched — external authors remain source provenance, never converted into an ICAS role. Because this is the one shared helper both CG Version Review and Artist Current Version already used, fixing it here fixed both named locations in one place; it also incidentally fixes the same pre-existing bug on the VFX Versions page (not one of the six priority pages, but the same shared, previously-buggy code path — its own pre-existing tests were updated to match, §13).

### 19.4 Artist Feedback History: server-generated Decision-event text

**Symptom:** `"Human cg_supervisor confirmed Execution Anchor Revision 2 -- this Task's operational boundaries"` — the raw enum is embedded inside the persisted `summary` string itself (`artist_feedback_history/service.py`), not held in a separate field the frontend could reformat by simply relabelling.

**Correction — frontend-only, no backend or contract change:** new `apps/web/src/lib/feedbackEventSummary.ts::feedbackEventSummary(event)`. For exactly the two event types whose summary is known to embed the role (`execution_anchor_confirmed`, `execution_anchor_draft_discarded`), it checks whether `event.summary` starts with the exact, structurally-known prefix `"Human ${event.actor_human_role} "` (using the event's own real `actor_human_role` field, never a guess) and, only if so, replaces that prefix with `` `${humanRoleLabel(event.actor_human_role)} ` `` — e.g. `"CG Supervisor confirmed Execution Anchor Revision 2 -- this Task's operational boundaries"`. Every other event type's summary (Version/ReviewNote/Dependency text, Agent-output descriptions) is returned completely unchanged; if the expected prefix is not found (a future backend format change, or `actor_human_role` is null), the original summary is returned unchanged rather than guessed at. This is a narrow, targeted substring match on a known, structurally-verified prefix — not a broad regex over arbitrary content, and it never modifies the persisted `summary` value itself, only what this one page renders. `apps/web/src/app/artist/tasks/[taskId]/feedback-history/FeedbackHistoryPage.tsx` now calls `feedbackEventSummary(event)` instead of rendering `event.summary` directly.

Per this task's own instruction ("no backend or contract change should be necessary; if one appears necessary, stop and report before changing it"), the backend's own `summary`-generation code was read and left completely unchanged — no Python file in this correction.

### 19.5 Tests and broadened re-verification

New/updated tests are listed per file in §13, including two new shared-helper test files (`authorProvenance.test.ts`, `feedbackEventSummary.test.ts`) directly covering the required cases: a real internal `HumanRole` formats correctly; a non-role `actor_kind` fallback is never converted; a real ftrack external author name is never treated as an ICAS role, including one that happens to contain a role-like substring; an unrecognised role value passes through unchanged; the two Decision-event types compose a readable description while every other event type's summary (explicitly including ReviewNote content) is asserted unchanged. All 964 frontend tests pass (123 files); typecheck, ESLint, Prettier, and production build all clean (§13).

`apps/api`/`apps/web` restarted fresh; all six §15 URLs (plus VFX Versions, as a spot-check of the shared `authorProvenance.ts` fix) returned `200`. This pass's re-verification is deliberately broader than §18.6's: rather than only confirming the specific strings the correction targeted, it searched the full rendered HTML of all seven pages for `>vfx_supervisor<`, `>cg_supervisor<`, `>artist<`, and mixed-case variants as literal element text (as opposed to internal RSC hydration payload data, which legitimately still carries the real enum value for React's own use and is not user-visible) — **zero matches on every page.** The four corrected strings were independently confirmed present and correctly formatted: `"Affects: VFX Supervisor, CG Supervisor, Artist"` (VFX Alignment); `"VFX Supervisor"`/`"CG Supervisor"` review-note/version author text (CG Version Review, Artist Current Version); `"CG Supervisor confirmed Execution Anchor Revision 1"` and `"...Revision 2"` (Artist Feedback History, both real events from the D1 demo history).
