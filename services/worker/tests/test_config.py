...
from intent_core_worker.config import Settings
...
def test_env_file_is_an_absolute_path_to_the_repo_root_dotenv() -> None:
    ...
    defaults otherwise (e.g. `cd services/worker && uv run ...`,
    matching this package's own README). The absolute path must
    resolve to the same `.env` regardless of invocation cwd.
    """
